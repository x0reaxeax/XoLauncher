# ![XoLauncher](https://i.imgur.com/reKe6vG.png) XoLauncher
### 1. Setting up XoLauncher  
   **1.1** Download [XoLauncher](https://github.com/x0reaxeax/XoLauncher/releases/download/rls/XoLauncher.rar) and extract it  
   **1.2** Open **XoLauncher** and navigate the first-time setup to your MultiMC directory  
### 2. Launching Minecraft  
   **2.1** Select your MultiMC instance and press `Launch Minecraft`  


![XoLauncher](https://i.imgur.com/sMWxf93.jpg)

## Info
  * XoLauncher automatically syncs XoPack mods with the server, so you don't have to move a finger if we ever add a new mod!
  * That's about it actually..  
  * XoLauncher requires a functional MultiMC instance with ~~**`Forge 14.23.5.2816`** (compatible up-to `14.23.5.2838`)~~ **`Forge 14.23.5.2847`**
  * XoPack is a **REALLY BIG** modpack (over 100+ mods)! You might need a beefy computer to run it..
  

## TO-DO:
- [ ] Dynamic Download Progress Bar
- [ ] **Fix bug with launching incorrect instance after (???)**
- [ ] Fix broken parsing when downloading directly from CurseForge
- [x] Automatic removal of outdated mods
- [x] Fix bug with launching incorrect instance after re-launching Minecraft
- [x] Add title font to resources file
 
 ## Changelog:
  **v1.1**
   * Added mechanism to delete outdated mods
   * Added Update notification on startup
   * Fixed bug with launching incorrect instance after re-launching Minecraft
   * Added title font to resources file
 
 ## MOTD:
   * ~~Added `EnderIO` and `Refined Storage`~~
   * Added `GalacticCraft + Addons & MorePlanets` + `Gauges and Switches`
   * Updated 36 mods and added `ICBM - Classic`, Electroblob's Wizardry & `BlockCraftery` (as an 1.12.2 alternative to Carpenter's Blocks)!
   * **Updated Forge to `14.23.5.2847`, please update Forge in MultiMC using `Install Forge` button in `Edit Instance` option.**
 
 [License](https://github.com/x0reaxeax/XoLauncher/blob/master/LICENSE)
 
 **x0reaxeax 2020**

  
  
  
    
### MOD LIST
```
AppleSkin
Avaritia
Baubles
BetterFps
bewitchment
BiblioCraft
BiomesOPlenty
BlackHoleStorage_1
BrandonsCore
Chameleon
Chisel
chiselsandbits
CodeChickenLib
CoFHCore
CoFHWorld
CommonCapabilities
CTM
CyclopsCore
Decocraft
deepresonance
Draconic
ElecCore
EnderCore
EvilCraft
extrautils2
ExtremeReactors
Flan
Forgelin
ForgeMultipart
furniture
FutWep
GardenStuff
geneticsreborn
Guide
HammerCore
Hwyla
ICBM-Classic
iChunUtil
industrialcraft
industrialforegoing
Inspirations
JustEnoughItems
JustEnoughReactors
llibrary
logisticalautomation
malisisblocks
malisiscore
malisisdoors
malisisswitches
Mantle
mcjtylib
MCMultiPart
meecreeps
Mekanism
MekanismGenerators
Morph
MrTJPCore
NotEnoughIDs
notenoughrtgs
NuclearCraft
OfflineSkins
OpenBlocks
OpenComputers
OpenModsLib
Patchouli
Placeable
PortalGun
practicallogistics2
ProjectRed
ProjectRed
ProjectRed
ProjectRed
ProjectRed
ProjectRed
ProjectRed
PTRLib
quantumflux
RedstoneFlux
rftools
rftoolsdim
SCP
secretroomsmod
SecurityCraft
SignPicture
simpleportals
sonarcore
TConstruct
Tesla
tesla
ThermalDynamics
ThermalExpansion
ThermalFoundation
twilightforest
Useful
VanillaFix
worldedit
xnet
zerocore
```
